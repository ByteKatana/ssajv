...
export const ListItem = (props) => {
  ...
  if (open) {
    ...
    props.job.activity
      .filter((val, key) => key <= 9)
      .map((act) => {
        if (act.start_execution_date) {
          //reformat activity execution date to be able to compare with history run date
          let activityDate = `${act.start_execution_date.slice(
            0,
            4
          )}${act.start_execution_date.slice(5, 7)}${act.start_execution_date.slice(
            8,
            10
          )}`
          //reformat activity execution date to show in line chart as Month-DayTH format
          let formattedActivityDate = `${moment(
            act.start_execution_date.slice(0, 10),
            "YYYY-MM-DD"
          ).format("MMM Do")}`

          props.job.history
            .filter((ht) => ht.step_id == "0" && ht.run_date.toString() === activityDate)
            .map((ht) => {
    ...
    //
    durationChartData.map(
      (chartData) => (chartData.data = _.reverse(tempActivityChartData))
    )
  }
  ...
  return (
    <>
      <tr onClick={() => setOpen(!open)} aria-controls={`test`} aria-expanded={open}>
        <td>
          {props.job.enabled == 1 ? (
            <FiCheck color={"#28a745"} size={"24px"} />
          ) : (
          ...
          <OverlayTrigger
            ...
            overlay={
              <Tooltip id="tooltip-disabled">
                <b>
                  <i>{props.job.servers[0].last_outcome_message}</i>
                </b>
              </Tooltip>
            }>
            <span className="d-inline-block">
              <LastRunOutCome
                isColored={true}
                outcome={props.job.servers[0].last_run_outcome}
              />
            </span>
          </OverlayTrigger>
        </td>
        <td>
          <Moment parse="YYYY-MM-DDTHH:mm:ss" format="DD/MM/YYYY HH:mm:ss">
            {props.job.servers[0].last_run_timestamp}
          </Moment>
        </td>
        <td>
          <Moment parse="YYYY-MM-DDTHH:mm:ss" format="DD/MM/YYYY HH:mm:ss">
            {props.job.schedules
              .slice(0, 1)
              .map((schedule) => schedule.next_run_timestamp)}
          </Moment>
        </td>
      </tr>
                    ...
                    <Table bordered hover responsive className={"text-center"}>
                      ...
                      <tbody>
                        {props.job.activity.map((act) => {
                          if (act.start_execution_date) {
                            let activity_date = `${act.start_execution_date.slice(
                              0,
                              4
                            )}${act.start_execution_date.slice(
                              5,
                              7
                            )}${act.start_execution_date.slice(8, 10)}`
                              ...
                              <>
                                {props.job.history
                                  .filter(
                                    (log) =>
                                      log.step_id == "0" &&
                                      log.run_date.toString() === activity_date
                                  )
                                  .map((log) => {
                                    return (
                                      <History
                                        key={log.instance_id}
                                        start_exec_time={act.start_execution_date}
                                        history={log}
                                        subHistory={props.job.history.filter(
                                          (lg) =>
                                            lg.step_id != 0 &&
                                            lg.run_date.toString() === activity_date &&
                                            lg.run_time >= log.run_time &&
                                            lg.run_time <=
                                              Number(log.run_time) +
                                                Number(log.run_duration)
                                        )}
                                      />
                                    )
                                  })}
